...
function buildUserMessage(prompt: string, previousMermaid: string, language: string): string {
  const langNote = language === 'hu' ? ' (The user writes in Hungarian but entity/class names must still be in English.)' : '';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { prompt, model, apiKey, ollamaModel, language, previousMermaid = '' } = body;
    ...
    if (model === 'cloud') {
      if (!apiKey?.trim()) {
        return NextResponse.json({ error: 'Gemini API key is required for Cloud mode.' }, { status: 400 });
      }
      mermaidCode = await generateWithGemini(prompt, previousMermaid, apiKey, language);
    } else {
      mermaidCode = await generateWithOllama(prompt, previousMermaid, ollamaModel ?? 'llama3', language);
    }

    if (!validateMermaid(mermaidCode)) {
      return NextResponse.json(
        { error: 'The AI did not return valid Mermaid code. Please try again with a more specific prompt.' },
        { status: 422 }
      );
    }
    ...
    console.error('[generate-diagram]', message);
